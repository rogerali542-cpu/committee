...
function ownerTotal(m) {
  // 优先用会议配置的总户数，否则从房屋花名册取
  if (m && m.totalOwners) return m.totalOwners;
  var units = require('../mock').housingUnits;
  return units && units.length ? units.length : 10;
}

function ownerArea(m) {
  // 优先用会议配置的总面积，否则从房屋花名册汇总
  if (m && m.totalArea) return m.totalArea;
  var units = require('../mock').housingUnits;
  if (units && units.length) {
    return units.reduce(function (s, u) { return s + (u.area || 0); }, 0);
  }
  return 930; // 10户兜底面积
}

// 获取某户代表人 userRoleId（用于投票校验）
function getHouseholdRep(unitId) {
  var units = require('../mock').housingUnits;
  var unit = units.find(function (u) { return u.id === unitId; });
  return unit ? unit.representativeUserId : null;
}

// 判断某人是否是某户的代表人
function isHouseholdRep(unitId, userRoleId) {
  var units = require('../mock').housingUnits;
  var unit = units.find(function (u) { return u.id === unitId; });
  return unit && unit.representativeUserId === userRoleId;
}

// 获取用户关联的所有房屋
function getUserUnits(userRoleId) {
  var units = require('../mock').housingUnits;
  return units.filter(function (u) { return u.ownerUserIds.indexOf(userRoleId) >= 0; });
}

// 获取用户代表的所有房屋（投票权）
function getRepresentedUnits(userRoleId) {
  var units = require('../mock').housingUnits;
  return units.filter(function (u) { return u.representativeUserId === userRoleId; });
}

function ensureOwnerNotify(m) {
  if (!m.notify) m.notify = { sent: 0, announced: false, contentComplete: false };
  return m.notify;
}

function ensureOwnerBallot(m) {
  if (!m.ballot) m.ballot = { delivered: 0, records: false, nonFaceAnnounce: false };
  return m.ballot;
}

function ensureOwnerRecord(m) {
  if (!m.record) {
    m.record = {
      presentOwners: 0,
      presentArea: 0,
      ...
      process: { designated: false, monitor: false, callout: false, tally: false },
      topics: [],
      evidence: []
    };
  }
  if (!m.record.process) m.record.process = { designated: false, monitor: false, callout: false, tally: false };
  if (!m.record.topics) m.record.topics = [];
  if (!m.record.evidence) m.record.evidence = [];
  return m.record;
}

function ownerStatusLine(m) {
  if (m.stage === C.STAGE.PREPARING) {
    var n = ensureOwnerNotify(m);
    return '通知进度 ' + n.sent + '/' + ownerTotal(m) + ' 户';
  }
  if (m.stage === C.STAGE.ONGOING) return '大会进行中';
  if (m.compliance === C.COMPLIANCE.INVALID) return '大会未成立';
  return m.publish && m.publish.published ? '已公示（' + m.publish.publishDate + '）' : '待公示';
}
...
function buildOwnerNoticeDraft(m) {
  var typeText = m.type === C.OWNER_TYPE.SPECIAL ? '临时业主大会' : '业主大会';
  var title = '关于召开' + (m.title || typeText) + '的通知';
  var lines = [
    title,
    '会议时间：' + (m.meetingDate || '待定') + ' ' + (m.meetingTime || ''),
    '会议地点：' + (m.location || '待定'),
    '主要事项：' + (m.description || '待补充'),
    m.needsVote === false ? '本次大会不涉及表决事项。' : '本次大会涉及表决事项，请业主按通知要求参与。',
    '请业主关注后续公告及材料送达安排。'
  ];
  return { title: title, content: lines.filter(Boolean).join('\n'), status: 'draft' };
}
...
function calcOwnerStats() {
  return {
    annualCount: mock.ownerMeetings.filter(function (m) { return m.type === C.OWNER_TYPE.REGULAR && m.stage === C.STAGE.ENDED && m.compliance !== C.COMPLIANCE.INVALID; }).length,
    specialCount: mock.ownerMeetings.filter(function (m) { return m.type === C.OWNER_TYPE.SPECIAL; }).length,
    preparing: mock.ownerMeetings.filter(function (m) { return m.stage === C.STAGE.PREPARING; }).length,
    ongoing: mock.ownerMeetings.filter(function (m) { return m.stage === C.STAGE.ONGOING; }).length,
    ended: mock.ownerMeetings.filter(function (m) { return m.stage === C.STAGE.ENDED && m.compliance !== C.COMPLIANCE.INVALID; }).length
  };
}
