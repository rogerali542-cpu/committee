    ...
    <main class="records-body">
      <section class="summary-grid">
        ...
        <div class="summary-card handled">
          <strong>{{ annualHandledCount }}</strong><span>已办理</span>
        </div>
        <div class="summary-card pending">
          <strong>{{ annualPendingCount }}</strong><span>待办理</span>
        </div>
      </section>
            ...
            <div v-for="record in day.records" :key="record.id" class="record-row" @click="goDetail(record)">
              <div class="record-main">
                <div class="record-title">
                  <strong>{{ record.visitorName === '无人来访' ? '无人来访' : (record.visitorName || '来访居民') }}</strong>
                  <span v-if="record.room">{{ record.room }}</span>
                  <i :class="recordStatus(record)">{{ statusLabel(recordStatus(record)) }}</i>
                </div>
                <p>{{ record.visitorName === '无人来访' ? '本次接待无居民来访，已完成留档' : (record.content || '无诉求内容') }}</p>
...
function statusLabel(status) {
  return status === 'done' ? '已办结' : (status === 'doing' ? '已办理' : '待处理')
}
