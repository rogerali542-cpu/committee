<template>
  <div class="page" :class="{ 'has-bar': canCreate }" style="overflow-y:auto">
    ...
    <div class="learn-target">
      <div class="lt-head">
        <span class="lt-icon" aria-hidden="true">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
            <path d="M12 6.5C10.5 5 8.2 4.4 5.5 4.4c-.8 0-1.5.6-1.5 1.4v11c0 .8.7 1.4 1.5 1.4 2.7 0 5 .6 6.5 2.1 1.5-1.5 3.8-2.1 6.5-2.1.8 0 1.5-.6 1.5-1.4v-11c0-.8-.7-1.4-1.5-1.4-2.7 0-5 .6-6.5 2.1z"/>
            <line x1="12" y1="6.5" x2="12" y2="20.3"/>
          </svg>
        </span>
      ...
      <div class="annual-grid">
        <div class="annual-row">
          ...
          <span v-else class="annual-status" :class="annualStudyCount >= target ? 'ok' : 'warn'">{{ annualStudyCount }} / {{ target }} 次</span>
        </div>
        <div class="annual-row">
          <span class="annual-name">外部培训</span>
          <span v-if="isLoading" class="annual-status ok">读取中…</span>
          <span v-else class="annual-status" :class="annualExternalCount >= 1 ? 'ok' : 'warn'">{{ annualExternalCount >= 1 ? ('已达标 · ' + annualExternalCount + '次') : '暂未参加' }}</span>
        </div>
      </div>
    </div>

    <!-- 最近一项：只表达近期安排；没有安排是正常业务空态，不与数据加载失败混淆 -->
    <section v-if="isLoading || nextItem" class="learn-section">
      <h2>当前安排</h2>
      <div v-if="isLoading" class="learning-empty">正在读取近期安排…</div>
      <div v-else class="next-learning" @click="tapItem(nextItem)">
    ...
    <section class="learn-section records-section">
      <div class="section-head">
        <h2>{{ currentYear }}年记录</h2>
      </div>
      <div v-for="it in recordItems.slice(0, 2)" :key="it.id" class="record-row" @click="openDetail(it)">
        <span class="record-main">
          <b>{{ it.title }}</b>
          <em><span class="record-kind">{{ it.category === 'external' ? '外部' : '内部' }}</span> · {{ fmtD(it.date) }}</em>
        </span>
        <i class="ll-arr"></i>
      </div>
      ...
      <button type="button" class="view-records" @click="goArchive('learning')">
        <span>往年记录</span>
        <i class="ll-arr"></i>
      </button>
    </section>

    <!-- 外部培训是低频补录，采用与“发起临时会议”一致的轻列表入口 -->
    <div v-if="canCreate" class="learn-links">
      <div class="learn-link" @click="openCreate">
        <span class="ll-text">＋ 登记外部培训</span>
        <i class="ll-arr"></i>
      </div>
    </div>

    <!-- 内部学习是本页主流程，保留为唯一固定主按钮 -->
...
async function loadAll() {
  ...
  try {
    ...
    const currentYearItems = merged.filter(i => String(i.date || '').startsWith(currentYearPrefix));
    annualStudyCount.value = currentYearItems.filter(i => i.category === 'internal' && i.stage === 'ended').length;
    annualExternalCount.value = currentYearItems.filter(i => i.category === 'external' && i.stage === 'ended').length;
...
// 首页只展示本年度记录；跨年度材料统一从“往年记录”进入档案馆查看。
const recordItems = computed(() => allItems.value.filter(i =>
  i.stage === 'ended' && String(i.date || '').startsWith(String(currentYear) + '-')
))
...
.page.has-bar { padding-bottom: 184rpx; }
...
/* 年度情况改为并排统计块：信息不减少，缩短纵向距离 */
.learn-target { margin: 20rpx 0 8rpx; overflow: hidden; background: #fff; border-radius: 24rpx; padding: 8rpx 28rpx 24rpx; box-shadow: 0 2rpx 6rpx rgba(31,41,55,.05), 0 10rpx 26rpx rgba(31,41,55,.07); }
.lt-head { display: flex; align-items: center; gap: 14rpx; min-height: 76rpx; }
.lt-icon { display: inline-flex; width: 34rpx; height: 34rpx; flex: 0 0 auto; align-items: center; justify-content: center; color: #2A6B73; }
.lt-icon svg { width: 34rpx; height: 34rpx; }
.lt-title { font-size: 31rpx; font-weight: 700; color: #1f2329; }
.annual-grid { display: flex; flex-direction: column; }
.annual-row { display: flex; min-width: 0; min-height: 136rpx; padding: 0 4rpx; box-sizing: border-box; align-items: center; justify-content: space-between; gap: 24rpx; border-bottom: 2rpx solid #E7E9ED; background: transparent; }
.annual-row:last-child { border-bottom: 0; }
.annual-name { font-size: 38rpx; font-weight: 700; color: #1F2937; }
/* 三级色（规范§四）：未达标=暖胶囊（唯一跳出来的），达标=无底灰字 */
.annual-status { margin-left: auto; max-width: 100%; font-size: 38rpx; font-weight: 600; text-align: right; white-space: nowrap; }
.annual-status.ok { color: #6B7280; font-weight: 500; }
.annual-status.warn { color: #B56516; background: transparent; padding: 0; border-radius: 0; }

/* 轻列表（62px 行高标准，与首页/会议页一致）：透明底+分隔线 */
.learn-links { margin-top: 0; padding: 0 8rpx; }
.learn-link { display: flex; align-items: center; justify-content: space-between; gap: 12rpx; min-height: 104rpx; cursor: pointer; }
.learn-link:last-child { border-bottom: 2rpx solid #E2E5EA; }
.learn-link:active { opacity: .65; }
.ll-text { flex: 1; min-width: 0; font-size: 30rpx; font-weight: 600; color: #1F2937; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
...
.learn-section.empty-section { min-height: 92rpx; padding: 22rpx 28rpx; display: flex; align-items: center; justify-content: space-between; gap: 20rpx; }
.learn-section.empty-section .learning-empty { flex-shrink: 1; margin: 0; padding: 0; font-size: 26rpx; text-align: right; }
.learn-section.records-section { padding: 0 8rpx; background: transparent; border-radius: 0; box-shadow: none; }
.records-section .section-head { min-height: 104rpx; margin: 0; padding: 0 4rpx; border-bottom: 2rpx solid #E2E5EA; }
.records-section .section-head h2 { font-size: 30rpx; font-weight: 600; }
.records-section .section-head > span { font-size: 26rpx; }
...
.record-row { display: flex; align-items: center; gap: 12rpx; min-height: 128rpx; padding: 18rpx 0; box-sizing: border-box; border-bottom: 2rpx solid #E2E5EA; cursor: pointer; }
.record-main { flex: 1; min-width: 0; display: flex; flex-direction: column; gap: 8rpx; }
.record-main b { color: #1F2937; font-size: 29rpx; font-weight: 550; line-height: 1.42; white-space: normal; word-break: auto-phrase; overflow-wrap: break-word; text-wrap: pretty; }
.record-main em { color: #7A8594; font-size: 25rpx; font-style: normal; }
.record-kind { color: #6B7280; font-weight: 600; }
...
.view-records { display: flex; width: 100%; min-height: 104rpx; padding: 0 4rpx; align-items: center; gap: 14rpx; border: 0; border-bottom: 2rpx solid #E2E5EA; background: transparent; color: #1F2937; font-size: 30rpx; font-weight: 600; text-align: left; }
.view-records > span { flex: 1; }
.view-records > em { color: #8792A2; font-size: 25rpx; font-style: normal; font-weight: 500; white-space: nowrap; }

/* 内部学习保留为唯一固定主按钮；外部培训使用上方轻列表入口 */
...
.learn-primary { width: 100%; min-height: 108rpx; border-radius: 20rpx; font-size: 34rpx; font-weight: 700; display: flex; align-items: center; justify-content: center; border: 0; background: #2a6b73; color: #fff; }
