          ...
          <div v-for="c in portalWork" :key="c.key" class="pt-sec"
            ...
            <div class="pt-sec-row">
              <div class="pt-sec-main">
                <div class="pt-sec-title" :class="titleFit(c.title)">{{ c.title }}</div>
...
// 标题自适应字号（0731 用户定：内容显示不全时字体自动缩小一号，白卡高度不超定值）。
// 按渲染宽度加权估算：中日韩全宽字符记 1、数字/字母/冒号记 0.55、空格 0.3——
// 「19:00—20:00」这类数字串实际只占约半宽，按字符数硬算会把短标题误降号。
// 降号后仍放不下的极长标题由 CSS 折到第 2 行、两行封顶省略。
function titleFit(t) {
  let w = 0
  for (const ch of String(t || '')) {
    if (ch === ' ') { w += 0.3; continue }
    w += /[⺀-鿿豈-﫿＀-￯–—]/.test(ch) ? 1 : 0.55
  }
  return w > 16 ? 'fit-s' : w > 13 ? 'fit-m' : ''
}
...
/* 正文 20px（0731 设计师定：行放大）＝该卡主读信息。0731 用户定：不再单行硬截——内容长时
   titleFit 降号（40→34→31rpx），仍放不下折到第 2 行、两行封顶省略，白卡高度不超定值 */
.pt-sec-title { font-size: 40rpx; font-weight: 600; color: #1F2937; line-height: 1.35; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; word-break: break-all; }
.pt-sec-title.fit-m { font-size: 34rpx; }
.pt-sec-title.fit-s { font-size: 31rpx; }
