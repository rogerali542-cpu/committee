  ...
  <div class="omf">
    ...
    <template v-else>
      ...
      <section v-if="step === 1" class="omf-card">
        ...
        <button class="omf-primary" :disabled="busy || !selectedMembers.length" @click="saveAttendance">
          {{ busy ? '正在保存…' : '下一步，填写议题结果' }}
        </button>
      </section>
...
function initFromDetail() {
  ...
  selectedMembers.value = attendance.value.filter(item => item.signedIn).map(item => Number(item.userRoleId))
...
async function saveAttendance() {
  busy.value = true
  try {
    await api.committeeSetOnlineAttendance(props.meetingId, selectedMembers.value)
    await emitReload()
    step.value = 2
  } catch (e) {
    toast({ title: e.message || '参会名单保存失败', icon: 'none' })
...
<style scoped>
.omf{padding:24rpx 8rpx 60rpx;color:#243746}.omf-head{display:flex;justify-content:space-between;align-items:flex-start;margin:12rpx 8rpx 26rpx}.omf-kicker{font-size:28rpx;color:#62788a}.omf-head h2{margin:8rpx 0 0;font-size:38rpx}.omf-platform{padding:8rpx 16rpx;border-radius:999rpx;background:#edf4f8;color:#52748d;font-size:26rpx}.omf-steps{display:flex;align-items:center;margin:0 8rpx 24rpx}.omf-steps span{font-size:26rpx;color:#96a3ad;white-space:nowrap}.omf-steps span.on{color:#315f7d;font-weight:700}.omf-steps span.done{color:#5d8b72}.omf-steps i{height:2rpx;flex:1;margin:0 12rpx;background:#dce4e9}.omf-card{padding:30rpx 28rpx;border:2rpx solid #e0e7eb;border-radius:22rpx;background:#fff;box-shadow:0 8rpx 28rpx rgba(45,66,80,.07)}.omf-card h3{margin:0;font-size:32rpx}.omf-desc{margin:12rpx 0 24rpx;color:#71808b;font-size:26rpx;line-height:1.65}.member-row{height:86rpx;display:flex;align-items:center;border-bottom:2rpx solid #eef2f4}.member-row input{display:none}.member-check{width:38rpx;height:38rpx;margin-right:18rpx;border:2rpx solid #cbd6dd;border-radius:10rpx;color:transparent;text-align:center;line-height:36rpx}.member-check.checked{background:#5f879f;border-color:#5f879f;color:#fff}.member-name{font-size:32rpx}.member-role{margin-left:auto;color:#84929b;font-size:26rpx}.omf-primary,.omf-secondary,.omf-end{border:0;border-radius:14rpx;height:76rpx;font-size:28rpx}.omf-primary{width:100%;margin-top:28rpx;background:#416f8b;color:#fff}.omf-primary:disabled{opacity:.45}.omf-secondary{background:#edf2f5;color:#5b6d78}.omf-actions{display:flex;gap:16rpx}.omf-actions .omf-secondary{width:30%;margin-top:28rpx}.omf-actions .omf-primary{width:70%}.topic-form{padding:24rpx 0;border-top:2rpx solid #edf1f3}.topic-form:first-of-type{border-top:0}.topic-form-head{display:flex;gap:14rpx;align-items:flex-start}.topic-no{width:38rpx;height:38rpx;border-radius:50%;background:#e7f0f5;color:#416f8b;text-align:center;line-height:38rpx}.topic-form-head b{display:block;font-size:28rpx}.topic-form-head small{display:block;margin-top:6rpx;color:#8a98a1}.topic-input-wrap{position:relative;margin-top:18rpx}.topic-input-wrap textarea{box-sizing:border-box;width:100%;padding:18rpx 18rpx 86rpx;border:2rpx solid #d9e2e7;border-radius:14rpx;font-size:28rpx;line-height:1.55;resize:none}.voice-fill{position:absolute;right:12rpx;bottom:12rpx;min-width:176rpx;height:62rpx;padding:0 24rpx;border:2rpx solid #bdd0dc;border-radius:14rpx;background:#dfeef6;color:#315f7a;font-size:28rpx;font-weight:600;box-shadow:0 4rpx 12rpx rgba(62,103,128,.13)}.voice-fill-icon{margin-right:8rpx}.voice-fill.recording{background:#f6dfdf;border-color:#e3bcbc;color:#a74343}.vote-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:12rpx;margin-top:16rpx}.vote-grid label{display:flex;align-items:center;padding:10rpx 12rpx;border-radius:12rpx;background:#f5f7f8}.vote-grid span{font-size:26rpx;color:#687984}.vote-grid input{width:50rpx;margin-left:auto;border:0;background:transparent;text-align:center;font-size:28rpx}.share-card{text-align:center}.share-icon{width:76rpx;height:76rpx;margin:0 auto 18rpx;border-radius:50%;background:#e4f2e9;color:#4d8a65;font-size:40rpx;line-height:76rpx}.full{width:100%;margin-top:14rpx}.confirm-progress{display:flex;justify-content:space-between;margin:28rpx 0 16rpx;padding:20rpx;border-radius:14rpx;background:#f5f7f8}.omf-end{width:100%;background:#fff;border:2rpx solid #8b9ba5;color:#485e6b}.omf-link{margin-top:20rpx;border:0;background:transparent;color:#6b8291}.result-card{text-align:left}.result-seal{text-align:center;font-size:34rpx;font-weight:700;color:#274c63}.result-meta{text-align:center;margin:10rpx 0 26rpx;color:#798892;font-size:26rpx}.result-attendance{padding:18rpx;border-radius:12rpx;background:#f4f7f8}.result-attendance b{display:block;margin-bottom:8rpx}.result-topic{padding:22rpx 0;border-bottom:2rpx solid #edf1f3}.result-topic-title{font-weight:700}.result-topic-text{margin-top:10rpx;color:#526570;line-height:1.65;white-space:pre-wrap}.result-votes{margin-top:10rpx;color:#49718a}.result-note{margin:24rpx 0 0;color:#75858f;font-size:26rpx}.result-confirmed{margin-top:26rpx;padding:20rpx;border-radius:14rpx;background:#e9f5ed;color:#43815b;text-align:center}
.option-vote-list{display:flex;flex-direction:column;gap:10rpx;margin-top:16rpx}.option-vote-list label{display:flex;align-items:center;padding:14rpx;border-radius:12rpx;background:#f5f7f8}.option-vote-list span{flex:1;font-size:26rpx}.option-vote-list input{width:74rpx;border:0;background:#fff;text-align:center;font-size:28rpx}.option-vote-list em{margin-left:8rpx;color:#7a8992;font-style:normal;font-size:26rpx}
.topic-heading{display:flex;align-items:center;gap:12rpx;min-width:0}.topic-heading b{min-width:0}.topic-kind{flex:none;padding:4rpx 12rpx;border-radius:999rpx;font-size:26rpx;font-weight:600;line-height:1.4}.topic-kind.vote{background:#f7eadf;color:#9a5d2e}.topic-kind.notice{background:#eee8f6;color:#715692}.topic-kind.discussion{background:#e7f0f6;color:#426f8c}
.result-card .omf-primary{display:block;width:70%;margin-left:auto;margin-right:auto}.result-edit-btn{display:block;width:70%;height:76rpx;margin:16rpx auto 0;border:2rpx solid #b9c8d1;border-radius:14rpx;background:#fff;color:#496474;font-size:28rpx}
.omf-card .attendance-title{font-size:35rpx}
.wechat-share-preview{display:flex;box-sizing:border-box;width:100%;margin:22rpx 0 0;padding:18rpx;border:2rpx solid #dce3e7;border-radius:16rpx;background:#fff;text-align:left;box-shadow:0 5rpx 18rpx rgba(45,64,76,.08)}.wsp-thumb{position:relative;flex:0 0 126rpx;width:126rpx;height:104rpx;overflow:hidden;border-radius:12rpx;background:linear-gradient(145deg,#326b89,#79a8bd);color:#fff}.wsp-emblem{position:absolute;left:16rpx;top:14rpx;width:42rpx;height:42rpx;border:2rpx solid rgba(255,255,255,.75);border-radius:50%;font-size:26rpx;line-height:42rpx;text-align:center}.wsp-lines{position:absolute;left:16rpx;right:16rpx;bottom:17rpx;height:18rpx;border-top:3rpx solid rgba(255,255,255,.72);border-bottom:3rpx solid rgba(255,255,255,.5)}.wsp-content{display:flex;flex:1;min-width:0;padding-left:18rpx;flex-direction:column}.wsp-content b{overflow:hidden;color:#273a46;font-size:28rpx;line-height:1.35;text-overflow:ellipsis;white-space:nowrap}.wsp-content span{display:-webkit-box;overflow:hidden;margin-top:7rpx;color:#74828b;font-size:26rpx;line-height:1.35;-webkit-box-orient:vertical;-webkit-line-clamp:2}.wsp-content small{margin-top:auto;color:#a0a9af;font-size:19rpx}.wechat-share-btn{display:flex;align-items:center;justify-content:center;gap:10rpx}.wechat-mark{width:34rpx;height:34rpx;border-radius:9rpx;background:#fff;color:#416f8b;font-size:19rpx;line-height:34rpx}
.share-instruction{margin:14rpx 0 0;color:#263746;font-size:28rpx;line-height:1.6}
</style>
