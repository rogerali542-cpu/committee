...
function Read-EnvValue([string]$file, [string]$key) {
  $line = Get-Content $file -Encoding UTF8 | Where-Object { $_ -match "^$key=" } | Select-Object -First 1
...
Write-Host '[2/4] Dumping local ywh_db (full snapshot)...'
...
Write-Host "[3/4] Importing into cloud $cn (overwrites existing tables)..."
$env:MYSQL_PWD = $cp
